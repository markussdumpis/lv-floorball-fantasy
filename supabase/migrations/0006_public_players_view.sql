...
select
  ...
  p.team,
  p.team_id,
  coalesce(p.price_final, p.price_computed, p.price) as price,
  p.price_final,
  p.price_computed,
  p.price_manual,
  p.points_total,
  p.games,
  coalesce(agg.games, 0) as fantasy_games,
  coalesce(agg.total_points, 0) as fantasy_total,
  case when coalesce(agg.games, 0) > 0 then coalesce(agg.total_points, 0) / agg.games else 0 end as fantasy_ppg
from players p
left join (
  select
    player_id,
